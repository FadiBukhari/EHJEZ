...
API.interceptors.response.use(
  ...
  (error) => {
    ...
    // Don't show error or redirect for initial auth verification
    // (this is expected when user is not logged in)
    const isAuthVerification = url.includes("/users/verify");
    ...
    if (status === 404 && !isAuthVerification) {
      toast.error("Invalid Credentials. Please try again.");
    } else if (status === 401 && !isAuthVerification) {
      // 401 means token is invalid or expired
      // Only show error and redirect if it's NOT the initial auth check
      toast.error("Session expired. Please login again.");
      authStore.logout();
      window.location.href = "/signin";
    } else if (status === 403 && !isOptionalFeature && !isAuthVerification) {
    ...
    } else if (status >= 500 && !isAuthVerification) {
      toast.error("Server error. Please try again later.");
    } else if (!isAuthVerification) {
